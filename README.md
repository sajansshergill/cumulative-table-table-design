...
Here's a diagrma of the high level pipeline design for this pattern:

<img width="783" height="317" alt="image" src="https://github.com/user-attachments/assets/f49162fd-5207-4d4a-93db-e423f9eb0684" />
...
After we have our daily metrics, we FULL OUTER JOIN yersterday's cumulative table with today's daily data and build our metric arrays for each user. This allows us to bring the new history in without having to scan all of it. **(a big performance boost)**

These metric arrays allows us to easily answer queries about the history of all users using things like ARRAY_SUM to calculate whatever metric we want on whatever time frame the array allows.
...
The problem with this is we're scanning 30 days of event data every day to produce these numbers. A pretty wasteful, yet simple pipeline. There should be a way where we only have to scan the event data and combine with the results from the previous 29 days, right? Can we make a data structure where a data scientist can query our data and easily know the number of actions a user took in the last N number of days?

This design is pretty simple with only 3 steps:

**The Daily table step**
- In this step we aggregate just the events of today to see who is daily active. The table schema is here
- This query is pretty simple and straight forward here
...
**The Cumulation step**
- The table schema for this step is here
- The query for this step is much more complex. It's here
  ...
  - Next we want to build the activity_array column. We only want activity_Array to store the data of the last 30 days
    - So we check to see if CARDINALITY(activity_arrya) < 30 to understand if we can just add today's value to the front to the array or do we need to slice an element off the
      end of the array adding today's value to the front
    - We need to perform COALESCE(t.is_active_today, 0) to put zero values into the array when a user isn't active
  - We follow a very similar pattern to how we built activity_array but for likes, commenst, and shares as well!
  - After we build all our metric arrays, calculating weekly and monthly activity is pretty straight forward
    ...
    - CASE WHEN ARRAY_SUM(SLICE(activity_arrya, 1, 7)) > 0 THEN 1 ELSE 0 END gives us weekly active since we only check the first 7 elements of the array (i.e the last 7 days)
...
**The DAG step**
- The example DAG can be found here
- Key things to remember for the DAG are:
  - Cumulative DAGs should always be depends_on_past: True
  - The Daily aggregation step need to be upstream of the cumulation step
